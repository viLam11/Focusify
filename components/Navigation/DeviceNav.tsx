...
export default function DeviceNav({ currentView }: { currentView: 'todolist' | 'pomodoro' | 'calendar' | 'streak' | 'setting' }) {
    ...
    return (
        <View className='h-24 bg-white justify-between flex flex-row mx-4'>
            ...
            <View className='flex flex-col items-center justify-center'>
                <TouchableOpacity className='items-center' onPress={() => router.push('/main/Pomodoro')}>
            ...
            <View className='flex flex-col items-center justify-center'>
                ...
                <Text className={`text-sm font-semibold mt-2 ${currentView == 'calendar' ? 'text-[#fd6246]' : 'text-gray-500'} `}>Calendar</Text>
            </View>
            <View className='flex flex-col items-center justify-center'>
